...
def test_color2gray(image, reference_gray):
    """Tests if numpy_color2gray works the way it should do
...
def test_color2sepia(image, reference_sepia):
    ...
    assert np.allclose(
        reference_sepia[0, 0], expected_pixel_value, filtered
    ), "Assertion error from test_color2sepia: Not equal"  # based on caching or other stuff most of the sepia tests works every other time???
