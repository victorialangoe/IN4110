"""numpy implementation of image filters"""
from __future__ import annotations

import numpy as np


def numpy_color2gray(image: np.array) -> np.array:
    """Convert rgb pixel array to grayscale

    Args:
        image (np.array)
    Returns:
        np.array: gray_image
    """

    gray_image = np.dot(image[..., :3], [0.21, 0.72, 0.07])
    gray_image = gray_image.astype(np.uint8)
    gray_image_3d = np.stack([gray_image, gray_image, gray_image], axis=-1)

    return gray_image_3d


def numpy_color2sepia(image: np.array, k: float = 1) -> np.array:
    """Convert rgb pixel array to sepia

    Args:
        image (np.array)
        k (float): amount of sepia (optional)

    The amount of sepia is given as a fraction, k=0 yields no sepia while
    k=1 yields full sepia.

    (note: implementing 'k' is a bonus task,
        you may ignore it)

    Returns:
        np.array: sepia_image
    """
    if k > 1 or k < 0:
        raise ValueError(f"k needs to be within range 0 to 1. k is {k}")

    sepia_matrix = np.array(
        [
            [0.393, 0.769, 0.189],
            [0.349, 0.686, 0.168],
            [0.272, 0.534, 0.131],
        ]
    )

    sepia_image = np.dot(image[..., :3], sepia_matrix.T)
    sepia_image = np.clip(sepia_image, 0, 255).astype(np.uint8)

    if k == 0:
        return sepia_image

    blended_image = k * sepia_image + (1 - k) * image[..., :3]
    blended_image = np.clip(blended_image, 0, 255).astype(np.uint8)

    return blended_image
